...
문서형(Document): JSON/BSON 형태로 데이터 저장, 유연한 구조 (예: MongoDB)
키-값형(Key-Value): 키를 기준으로 데이터를 빠르게 조회 (예: Redis)
열형(Column-Family): 열 단위로 데이터 저장, 대규모 분석에 적합 (예: Cassandra)
그래프형(Graph): 노드-엣지 구조로 복잡한 관계 분석에 적합 (예: Neo4j)


**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**

스키마 구조:몽고DB는 동적 스키마, RDB는 고정 스키마

데이터 저장 형식:몽고DB는 BSON 문서, RDB는 테이블 기반

확장성:몽고DB는 수평 확장(샤딩), RDB는 보통 수직 확장

질의 방식:몽고DB는 자바스크립트 기반 쿼리, RDB는 SQL 사용

관계 표현:RDB는 정규화, 몽고DB는 중첩 문서로 관계 표현
