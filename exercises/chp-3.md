...
7. 후보키 중에서 적절한 기본키 선정이 어려울 경우, 일련번호 같은 인위적인 속성을 추가로 생성하여 지정하는 기본키를 (대리)라고 한다.
...
릴레이션 스키마는 특정 릴레이션의 논리적 구조를 의미하며, 릴레이션 인스턴스는 앞서 언급한 릴레이션 스키마에 따라 각 속성에 대응하는 임의의 속성 값을 가진 투플들의 집합이다.
**19 관계형 데이터 모델의 무결성 제약 조건 4가지를 쓰고 간략히 설명하시오.**
1. 개체 무결성 제약 조건 : 기본키로 지정된 모든 속성은 널 값을 가질 수 없으며, 릴레이션 내에서 중복되지 않는 유일한 값을 가져야 한다.
2. 참조 무결성 제약 조건 : 외래키로 지정된 속성은 참조하는 릴레이션의 기본키 값과 일치하거나 널 값만 가질 수 있다.
3. 도메인 무결성 제약 조건 : 투플의 모든 속성 값이 각 속성의 도메인에 속한 값만을 취해야 한다.
4. 유일성 제약 조건 : 모든 키 속성 값이 서로 중복되지 않고 유일해야 한다.
**20 후보키와 슈퍼키, 기본키 사이의 관련성에 대해 설명하시오.**
슈퍼키는 유일성을 보장하는 속성들의 집합이다. 후보키는 최소한으로 유일성을 보장하는 속성들의 집합이며, 슈퍼키의 부분집합이다. 기본키는 후보키 중에서 선택된 유일한 키다.
...
1. 투플의 유일성 : 릴레이션 내의 모든 투플은 서로 달라야 한다.
2. 투플의 무순서성 : 릴레이션에서 투플 간의 순서는 의미가 없다.
3. 속성의 무순서성 : 릴레이션의 속성들 간에도 순서는 의미가 없다. 속성 이름을 통해 속성 값을 참조하기 때문에 속성의 순서는 중요하지 않다.
4. 속성의 원자성 : 릴레이션의 모든 속성 값은 더 이상 분해할 수 없는 원자 값(atomic value)이어야 한다.
