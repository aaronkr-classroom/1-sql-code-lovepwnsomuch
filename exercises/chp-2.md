...
10. 개념적 스키마에 영향이 없도록 하면서 데아터베아스의 물리적 구조를 변경할 수 있는 것을(데이터 독립)이라한다.
...
데이터 사전 사용자와 시스템 관리를 위해 사용되기에 사용자 접근이 가능하지만, 데이터 디렉토리는 사용자 접근이 불가능하다.
**20 다양한 수준의 추상화와 테이터 독립성율 제공하기 위하여 대이터베이스 관리 시스템 (DBMS)에서 지원하는 3 단계 스키마 구조에 대해셔 간략히 설명하시오.**
DBMS는 크게 외부 스키마, 개념 스키마, 내부 스키마로 나뉘며 외부 스키마는 사용자가 외부에서 바라보는 관점, 개념 스키마는 모든 사용자들의 관점, 내부 스키마는 저장 장치의 관점에서 데이터베이스의 구조를 정의한 것이다.
