	...
	SELECT 이름, 학년, 소속학과, 휴대폰번호 FROM 학생
	WHERE (학년 >= 1 AND 학년 <= 3) OR NOT (소속학과 = '컴퓨터'); -- BETWEEN 1 AND 3도 OK

	SELECT 이름, 학년, 소속학과 FROM 학생
	WHERE 소속학과 = '컴퓨터' OR 소속학과 = '정보통신'
	ORDER BY 학년 ASC; -- WHERE 소속학과 = '컴퓨터' OR '정보통신' 이렇게 하면 X
	...
	-- CRUD
	-- INSERT를 위해서 해서 다시 안할게요 ...

	UPDATE 학생 SET 학년 = 3 WHERE 이름 = '이은진';
	SELECT * FROM 학생;

	UPDATE 학생 SET 이름 = '이은진' WHERE 이름 = '이은진';
	UPDATE 학생 SET 학년 = 3 WHERE 이름 = '이은진';
	SELECT * FROM 학생;
