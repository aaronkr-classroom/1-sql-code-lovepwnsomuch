![image](https://github.com/user-attachments/assets/c106d2f0-d5ec-4516-b608-87aee9e51a72)# 4장 연습 문제
...
2. 관계 대수는 수학 집합 이론에 기반을 둔 (집합) 연산 그룹과 관계형 데이터 모델을 위해 추가된 (일반) 연산 그룹 2 개의 그룹으로 분류한다.
3. 집합 연산(set operation) 그룹은 합집합, 교집합, 차집합, (디비전) 연산을 포함한다.
4. (합집합) 가능하려면 두 릴레이션의 차수(속성 개수)가 같고 대웅하는 속성의 순서와 도메인이 같아야한다.
...
9. (프로젝션)은 자연 조인이 반환하는 결과 릴레이선 중에서 한쪽 릴레이션 속성만으로 한정하여 반환하는 제한적 자연 조인 연산이다.
...
답:
1. 자연 조인 (Natural Join): T1 ⨝ T2
2. 세타 조인 (Theta Join): T1 ⨝_{T1.C = T2.C} T2
3. 셀렉션 + 조인: σ_{A=1}(T1) ⨝ T2
4. 프로젝션 + 조인: π_{B,D}(T1 ⨝ T2)
...
1. 남학생의 이름과 성별 학년을 검색하시오. 답 : π이름, 성별, 학년 (σ성별 = '남'(학생))
2. 대이터베이스’를 수강하는 학생들의 학번과 이름을 검색하시오. 답 : π학번, 이름 (학생 ⨝ 수강 ⨝ σ과목명='데이터베이스'(과목))
